...
class TestComplexRsqrt(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data = {}
        inputs_data['param_real:0'] =  rng.uniform(-20.0, 20.0, param_real_shape_1).astype(np.float32) 
        inputs_data['param_imag:0'] = rng.uniform(-20.0, 20.0, param_imag_shape_1).astype(np.float32) 
        print(inputs_data['param_real:0'])
        print(inputs_data['param_imag:0'])
        return inputs_data
    ...
    test_data = [dict(shape=[1]),
                 ...
                 dict(shape=[1, 3, 224]),
                 dict(shape=[1, 3, 100, 128]),
                 dict(shape=[1, 3, 50, 100, 128])
                ]
