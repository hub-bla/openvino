...
class TestComplexEqual(CommonTFLayerTest):
    ...
    def create_complex_equal_net(self, x_shape, y_shape, ir_version, use_legacy_frontend):
        import tensorflow as tf
        ...
        with tf.compat.v1.Session() as sess:
            ...
            tf.raw_ops.Equal(x=x, y=y, name='Equal')
    ...
    @pytest.mark.precommit
    @pytest.mark.nightly
    def test_complex_equal(self, x_shape, y_shape, 
                   ie_device, precision, ir_version, temp_dir, use_legacy_frontend):
        self._test(*self.create_complex_equal_net(x_shape, y_shape, ir_version=ir_version,
                                          use_legacy_frontend=use_legacy_frontend),
                   ie_device, precision, ir_version, temp_dir=temp_dir,
                   use_legacy_frontend=use_legacy_frontend)
